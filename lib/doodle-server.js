...
var WebSocketServer, app, center, chokidar, client, delay, events, filename, fire, fs, http, lastTime, log, options, path, print_help, time, watch_files, wss, _ref, _ref1,
...
if ((_ref1 = options.http) === "true" || _ref1 === "yes" || _ref1 === "on") {
...
fire = function(filepath) {
  return delay(options.delay, function() {
    center.emit('update');
    return log("Update from:", filepath);
  });
};
...
watch_files.forEach(function(file) {
  ...
  return watcher.on("change", function(filepath) {
    var extname, thisTime;
    thisTime = time();
